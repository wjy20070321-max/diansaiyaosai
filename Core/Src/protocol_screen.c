...
#include "usart.h"            // USART 外设句柄声明，发送串口数据时要用到 huart6
#include "controller_mgr.h"   // 系统总上下文 g_sys，用于状态上报
#include "task_mgr.h"         // 任务管理器接口，用于读取任务状态
#include "region.h"           // 区域判断接口，用于根据当前球坐标判断所在区域
#include <stdio.h>            // snprintf() 用到的标准输入输出库
#include <string.h>           // memset() / memcpy() / strncmp() 用到的字符串处理库
#include <stdlib.h>           // strtoul() 用到的标准库函数

/* -------------------- 模块内静态全局变量 -------------------- */
/* 这些变量仅在本文件内部可见，用于保存串口屏解析结果和接收缓冲 */
static ScreenRxData_t g_screen;  // 当前解析得到的串口屏命令结构体
static char g_line[96];          // 一行文本命令接收缓冲区
static uint8_t g_idx = 0U;       // 当前行缓冲区写入索引
...
{
    uint8_t cnt = 0U;      // 当前已成功解析出的数字个数
    char *endptr;          // strtoul 的结束指针
    unsigned long v;       // 临时保存解析出的数值
    ...
    while (*s != '\0' && cnt < max_cnt)
    {
        ...
        /* 用 strtoul 解析一个无符号十进制数 */
        v = strtoul(s, &endptr, 10);

        /* 如果 endptr 没有前进，说明这里不是一个合法数字，停止解析 */
        if (endptr == s)
        {
            break;
        }
...
static void ProtocolScreen_ParseLine(char *line)
{
    ...
    else if (strncmp(line, "ROUTE=", 6) == 0)
    {
        ...
        if (len > 0U)
        {
            ...
            g_screen.route_valid = 1U;               // 标记为新路径命令有效
            g_screen.update_ms = g_sys_ms;
        }
    }
    /* -------------------- PASS=a,b,c,... -------------------- */
...
void ProtocolScreen_Init(void)
{
    ...
    memset(g_line, 0, sizeof(g_line));      // 清空一行文本缓冲区
    g_idx = 0U;                             // 重置当前接收位置
}
 ...
 *
 * @note 控制器模块通常通过这个接口读取串口屏解析后的命令状态。
 */
 ...
 *
 * @note 这个函数基于“按行解析”的思路工作：
 *       - '\r'：忽略
 *       - '\n'：表示一条命令结束，开始解析整行
 *       - 其他字符：依次写入 g_line 缓冲区
 *
 *       也就是说，串口屏命令格式本质上是“文本行协议”。
 */
 ...
 *
 * @note 当前通过 USART6 发送，发送超时时间为 50ms。
 *       一般用于：
 *       - 发送 READY 提示
 *       - 发送状态信息
 *       - 调试输出到串口屏
